/**
 * Created by huazhi on 2017/3/7.
 */
// 图片上传

var UPLOAD_URL = "/app/file/upload",
    SWF_URL =  '../../webuploader-0.1.5/js/Uploader.swf',
...
function upload1() {
    ...
    uploader.on('uploadSuccess', function (file, response) {
        if (!response.success) {
            ("#iconImg").siblings('.loading_icon').remove();
            $("#iconImg").show();
            ...
            uploader.makeThumb( file, function( error, src ) {
                if ( error ) {
                    $("#iconImg").replaceWith('<span>不能预览</span>');
                    return;
                }
    ...
    uploader.on( 'uploadBeforeSend', function(block, data ) {
        data.CSRFToken=ask.newCsrf("app");
    });
}
...
function upload2() {
    ...
    uploader.on('uploadSuccess', function (file, ret) {
        if (ret.status == 0) {
            ...
            $('#qrcode').val(ret.data.file.url);
            $("#img2").attr('src', ret.data.file.thumb);
            setTimeout(function () {
                $("#img2").show();
                $('#img2').siblings('.loading_icon').remove();
            }, 100);
...
function upload3() {
    ...
    uploader.on('uploadSuccess', function (file, ret) {
        if (ret.status == 0) {
            ...
            $('#open_qrcode').val(ret.data.file.url);
            $("#img3").attr('src', ret.data.file.thumb);
            setTimeout(function () {
                $("#img3").show();
                $('#img3').siblings('.loading_icon').remove();
            }, 100);
...
function upload4() {
    ...
    uploader.on('uploadSuccess', function (file, ret) {
        if (ret.status == 0) {
            ...
            $('#' + file.id).attr('data-img', ret.data.file.url);
            $('#' + file.id).find('.img4').attr('src', ret.data.file.thumb);
            attrImg();
            setTimeout(function () {
