...
public class FileUtils{
   ...
   */
  public static boolean createFile(String path,String fileName,String filecontent,String type) throws IOException {
    Boolean bool = false;
    String filenameTemp = path+"/"+fileName+"."+type;//文件路径+名称+文件类型
    File file = new File(filenameTemp);
    try {
      //如果文件不存在，则创建新的文件
      if(file.exists()){
        delFile(path,fileName,type);
      }
      file.createNewFile();
      bool = true;
   ...
   * @return
   */
  public static boolean delFile(String path,String fileName,String type){
    Boolean bool = false;
    String filenameTemp = path+"/"+fileName+"."+type;//文件路径+名称+文件类型
    File file  = new File(filenameTemp);
    try {
  ...
  public static void main(String[] args) throws IOException {
    ...
    createFile(new File("").getCanonicalPath(),uuid+"myfile", "我的梦说别停留等待,就让光芒折射泪湿的瞳孔,映出心中最想拥有的彩虹,带我奔向那片有你的天空,因为你是我的梦 我的梦","txt");
  }
